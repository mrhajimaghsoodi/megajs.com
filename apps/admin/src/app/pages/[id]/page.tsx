  ...
  const save = async () => {
    try {
      await adminFetch(`/admin/cms/pages/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ title, slug, summary, bodyMdx, status, template, locale, seo: { metaTitle, metaDescription } }),
      });
      setMsg(d.saved);
      await load();
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link href="/pages" className="text-sm underline-offset-4 hover:underline">← {d.back}</Link>
          <h1 className="mt-2 font-display text-3xl font-bold">{d.editPage}</h1>
        </div>
        <div className="flex gap-2">
          <Button variant="destructive" className="cursor-pointer" onClick={() => void remove()}>{d.delete}</Button>
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2"><Label>SEO title</Label><Input value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} /></div>
          <div className="space-y-2"><Label>SEO description</Label><Input value={metaDescription} onChange={(e) => setMetaDescription(e.target.value)} /></div>
        </div>
      </div>
    </div>
  );
