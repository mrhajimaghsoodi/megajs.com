---
title: "JavaScript: Types"
description: "JavaScript — Types"
order: 2
sources:
  - name: JavaScript Docs
    url: https://developer.mozilla.org/en-US/docs/Web/JavaScript
  - name: MDN
    url: https://developer.mozilla.org/
  - name: W3Schools
    url: https://www.w3schools.com/
---

# Types

This page is part of the **JavaScript** documentation track on MEGA JS.

It follows the same topic order you find on the official docs and complementary references (MDN / W3Schools where applicable), rewritten for clarity.

## What you will learn

- Core concepts for **Types**
- Practical code samples you can paste and adapt
- Notes that keep specialized terms in English when that is clearer

```js
// Example placeholder — expand per topic in this folder
console.log('JavaScript / types');
```

> Edit only the files under `content/docs/javascript/` to change this technology independently.
