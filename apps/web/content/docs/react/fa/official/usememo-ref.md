---
title: "مرجع useMemo"
description: "این صفحه cache کردن calculation پرهزینه همراه accurate نگه داشتن dependencyها را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "مرجع useMemo — مرجع رسمی react.dev | MEGA JS"
seoDescription: "مرجع useMemo (usememo ref): خلاصه رسمی react.dev برای React API Reference با Hooks، Components، React DOM، Compiler و Rules of React همراه تمرین کاربردی."
keywords: "React, مرجع useMemo, آموزش React, مرجع React, React useMemo Reference, useMemo Reference tutorial, usememo ref"
order: 8
track: official
sources:
...
مرجع useMemo خلاصه‌ای از ساختار رسمی react.dev/reference است. تمرکز آن روی cache کردن calculation پرهزینه همراه accurate نگه داشتن dependencyها است و هدف آن کمک به author برای خواندن API Reference بدون تبدیل کردن صفحه به dump کامل مستندات است.
...
- در مرجع useMemo، caveatهای official را با نسخه React، mode توسعه مثل StrictMode و test واقعی مقایسه کنید.
...
ادامه track رسمی: **مرجع useCallback**.
