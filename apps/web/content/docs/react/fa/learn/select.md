---
title: "Select در React"
description: "این صفحه کنترل option انتخاب‌شده با state را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "آموزش Select در React در مسیر W3Schools | MEGA JS"
seoDescription: "آموزش Select در React (select): مثال HTML با React CDN، JSX، تمرین live مسیر W3Schools، component، props، state و نکته Hooks همراه تمرین کاربردی."
keywords: "React, Select در React, آموزش React, مرجع React, React Select, Select tutorial, select"
order: 35
track: learn
sources:
...
Select در React درباره کنترل option انتخاب‌شده با state است. این درس بازنویسی آموزشی و مستقل از مسیر W3Schools React Tutorial است و مثال‌ها به شکل HTML کامل با React CDN و Babel نوشته شده‌اند تا در live editor اجرا شوند.

در تمرین Select در React فقط ظاهر UI را نبینید؛ render شدن component، تغییر state، props، warningهای Console و رفتار JSX در DOM را هم بررسی کنید.

## ایده اصلی

- `<select value={choice}>`: field کنترل‌شده select.
- `<option>`: یک value قابل انتخاب.
- `selected state`: state نگه‌دارنده option فعلی.
...
| --- | --- | --- |
| `<select value={choice}>` | field کنترل‌شده select | در Select در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `<option>` | یک value قابل انتخاب | در Select در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `selected state` | state نگه‌دارنده option فعلی | در Select در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |

## اشتباه‌های رایج

- کپی کردن Select در React بدون بررسی Console warning، key، dependency array یا update شدن state.
- mutate کردن object یا array در state به جای ساخت copy تازه با spread، map یا filter.
- فراموش کردن تست `<select value={choice}>` با keyboard، state خالی، StrictMode و browser هدف.

## تمرین کوتاه

یک فایل `select.html` بسازید، مثال اول را اجرا کنید و سپس یک بخش مرتبط با `<select value={choice}>` را تغییر دهید. اگر UI تغییر نکرد، Console و محل call شدن `root.render` یا update شدن state را بررسی کنید.

## گام بعدی

بعد از این صفحه، **چند input در React** را بخوانید تا مسیر React مرحله‌به‌مرحله کامل‌تر شود.
