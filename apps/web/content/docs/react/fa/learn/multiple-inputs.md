---
title: "چند input در React"
description: "این صفحه update کردن یک object state فرم از چند field را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "آموزش چند input در React در مسیر W3Schools | MEGA JS"
seoDescription: "آموزش چند input در React (multiple inputs): مثال HTML با React CDN، JSX، تمرین live مسیر W3Schools، component، props، state و نکته Hooks همراه تمرین کاربردی."
keywords: "React, چند input در React, آموزش React, مرجع React, React Multiple Inputs in React, Multiple Inputs in React tutorial, multiple inputs"
order: 36
track: learn
sources:
...
چند input در React درباره update کردن یک object state فرم از چند field است. این درس بازنویسی آموزشی و مستقل از مسیر W3Schools React Tutorial است و مثال‌ها به شکل HTML کامل با React CDN و Babel نوشته شده‌اند تا در live editor اجرا شوند.

در تمرین چند input در React فقط ظاهر UI را نبینید؛ render شدن component، تغییر state، props، warningهای Console و رفتار JSX در DOM را هم بررسی کنید.

## ایده اصلی

- `name`: شناسه field در handlerهای update.
- `computed property`: یک key از object را dynamic update می‌کند.
- `form state object`: یک object برای valueهای مرتبط فرم.
...
| --- | --- | --- |
| `name` | شناسه field در handlerهای update | در چند input در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `computed property` | یک key از object را dynamic update می‌کند | در چند input در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `form state object` | یک object برای valueهای مرتبط فرم | در چند input در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |

## اشتباه‌های رایج

- کپی کردن چند input در React بدون بررسی Console warning، key، dependency array یا update شدن state.
...
یک فایل `multiple-inputs.html` بسازید، مثال اول را اجرا کنید و سپس یک بخش مرتبط با `name` را تغییر دهید. اگر UI تغییر نکرد، Console و محل call شدن `root.render` یا update شدن state را بررسی کنید.

## گام بعدی

بعد از این صفحه، **Checkbox در React** را بخوانید تا مسیر React مرحله‌به‌مرحله کامل‌تر شود.
