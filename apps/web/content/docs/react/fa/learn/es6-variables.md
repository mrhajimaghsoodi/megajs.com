---
title: "Variableهای ES6"
description: "این صفحه انتخاب let و const برای code component و helperهای state را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "آموزش Variableهای ES6 در مسیر W3Schools | MEGA JS"
seoDescription: "آموزش Variableهای ES6 (es6 variables): مثال HTML با React CDN، JSX، تمرین live مسیر W3Schools، component، props، state و نکته Hooks همراه تمرین کاربردی."
keywords: "React, Variableهای ES6, آموزش React, مرجع React, React ES6 Variables, ES6 Variables tutorial, es6 variables"
order: 10
track: learn
sources:
...
Variableهای ES6 درباره انتخاب let و const برای code component و helperهای state است. این درس بازنویسی آموزشی و مستقل از مسیر W3Schools React Tutorial است و مثال‌ها به شکل HTML کامل با React CDN و Babel نوشته شده‌اند تا در live editor اجرا شوند.

در تمرین Variableهای ES6 فقط ظاهر UI را نبینید؛ render شدن component، تغییر state، props، warningهای Console و رفتار JSX در DOM را هم بررسی کنید.

## ایده اصلی

- `const`: binding که reassignment نمی‌شود.
- `let`: binding با block scope که می‌تواند تغییر کند.
- `scope`: جایی که binding دیده می‌شود.
...
| --- | --- | --- |
| `const` | binding که reassignment نمی‌شود | در Variableهای ES6 آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `let` | binding با block scope که می‌تواند تغییر کند | در Variableهای ES6 آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `scope` | جایی که binding دیده می‌شود | در Variableهای ES6 آن را با render واقعی، state خالی، interaction و Console تست کنید. |

## اشتباه‌های رایج

- کپی کردن Variableهای ES6 بدون بررسی Console warning، key، dependency array یا update شدن state.
...
یک فایل `es6-variables.html` بسازید، مثال اول را اجرا کنید و سپس یک بخش مرتبط با `const` را تغییر دهید. اگر UI تغییر نکرد، Console و محل call شدن `root.render` یا update شدن state را بررسی کنید.

## گام بعدی

بعد از این صفحه، **متد map در ES6** را بخوانید تا مسیر React مرحله‌به‌مرحله کامل‌تر شود.
