---
title: "Textarea در React"
description: "این صفحه کنترل text چندخطی با value و onChange را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "آموزش Textarea در React در مسیر W3Schools | MEGA JS"
seoDescription: "آموزش Textarea در React (textarea): مثال HTML با React CDN، JSX، تمرین live مسیر W3Schools، component، props، state و نکته Hooks همراه تمرین کاربردی."
keywords: "React, Textarea در React, آموزش React, مرجع React, React Textarea, Textarea tutorial, textarea"
order: 34
track: learn
sources:
...
Textarea در React درباره کنترل text چندخطی با value و onChange است. این درس بازنویسی آموزشی و مستقل از مسیر W3Schools React Tutorial است و مثال‌ها به شکل HTML کامل با React CDN و Babel نوشته شده‌اند تا در live editor اجرا شوند.

در تمرین Textarea در React فقط ظاهر UI را نبینید؛ render شدن component، تغییر state، props، warningهای Console و رفتار JSX در DOM را هم بررسی کنید.

## ایده اصلی

- `<textarea value={text}>`: input چندخطی کنترل‌شده.
- `rows`: hint ارتفاع قابل مشاهده textarea.
- `onChange`: updateهای text را دریافت می‌کند.
...
| --- | --- | --- |
| `<textarea value={text}>` | input چندخطی کنترل‌شده | در Textarea در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `rows` | hint ارتفاع قابل مشاهده textarea | در Textarea در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |
| `onChange` | updateهای text را دریافت می‌کند | در Textarea در React آن را با render واقعی، state خالی، interaction و Console تست کنید. |

## اشتباه‌های رایج

- کپی کردن Textarea در React بدون بررسی Console warning، key، dependency array یا update شدن state.
- mutate کردن object یا array در state به جای ساخت copy تازه با spread، map یا filter.
- فراموش کردن تست `<textarea value={text}>` با keyboard، state خالی، StrictMode و browser هدف.

## تمرین کوتاه

یک فایل `textarea.html` بسازید، مثال اول را اجرا کنید و سپس یک بخش مرتبط با `<textarea value={text}>` را تغییر دهید. اگر UI تغییر نکرد، Console و محل call شدن `root.render` یا update شدن state را بررسی کنید.

## گام بعدی

بعد از این صفحه، **Select در React** را بخوانید تا مسیر React مرحله‌به‌مرحله کامل‌تر شود.
