---
title: "مرجع Form کنترل‌شده"
description: "این صفحه input کنترل‌شده، مدیریت submit، feedback validation و ساختار state فرم را با مثال React، جدول syntax و نکته‌های کاربردی برای component authoring توضیح می‌دهد."
seoTitle: "مرجع Form کنترل‌شده — سبک MDN برای React | MEGA JS"
seoDescription: "مرجع Form کنترل‌شده (forms controlled): مرجع MDN-style برای React با تعریف author-focused، JSX، Hooks، مثال HTML و نکته پروژه واقعی همراه تمرین کاربردی."
keywords: "React, مرجع Form کنترل‌شده, آموزش React, مرجع React, React Controlled Forms Reference, Controlled Forms Reference tutorial, forms controlled"
order: 7
track: reference
sources:
...
مرجع Form کنترل‌شده یک صفحه مرجع author-focused به سبک MDN است. تمرکز آن روی input کنترل‌شده، مدیریت submit، feedback validation و ساختار state فرم است و به جای آموزش قدم‌به‌قدم، تعریف دقیق، جدول کاربرد، behavior در runtime و boundaryهای component را کنار هم می‌گذارد.

## جایگاه در MDN

در MDN، React در بخش Client-side JavaScript frameworks کنار مفهوم component، props، state، event، form، accessibility و testing توضیح داده می‌شود. هنگام استفاده از مرجع Form کنترل‌شده، رفتار React را از behavior خام DOM و ابزار build جدا ببینید.
...
- برای مرجع Form کنترل‌شده، data flow بین props، state و context را explicit نگه دارید.
...
این صفحه به موضوعات MDN درباره Controlled Forms Reference, React framework guides, accessibility و testing نزدیک است و برای مرور سریع قبل از پیاده‌سازی طراحی شده است.

## گام بعدی

صفحه مرجع بعدی: **مرجع SyntheticEvent**.
