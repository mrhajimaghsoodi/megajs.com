---
title: "Node.js: fs & path"
description: "Node.js — fs & path"
order: 3
sources:
  - name: Node.js Docs
    url: https://nodejs.org/docs/latest/api/
  - name: MDN
    url: https://developer.mozilla.org/
  - name: W3Schools
    url: https://www.w3schools.com/
---

# fs & path

This page is part of the **Node.js** documentation track on MEGA JS.

It follows the same topic order you find on the official docs and complementary references (MDN / W3Schools where applicable), rewritten for clarity.

## What you will learn

- Core concepts for **fs & path**
- Practical code samples you can paste and adapt
- Notes that keep specialized terms in English when that is clearer

```js
// Example placeholder — expand per topic in this folder
console.log('Node.js / fs-path');
```

> Edit only the files under `content/docs/node.js/` to change this technology independently.
